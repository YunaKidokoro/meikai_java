...
public class Ensyu7_19 {
	 ...
	 * 			int型・ｎ ・idxからいくつ削除するか
	 * 戻り値:int[]型・a[]・ある値からn個削除された配列
	 * 作成日:2023/04/21
	 * */
	...
	public static void main(String[]args) {
		...
		do{
			...
			if(howDeleteIndex>inputArray.length-deleteIndex-1) {
				//再入力を求める表示
				System.out.print("もう一度入力して下さい：");}
			}
		//削除したいインデックス数が配列要素数以上なら繰り返す
		while(howDeleteIndex>inputArray.length-deleteIndex-1);

		
		//配列から入力された配列要素を削除する
		int []deleteArray = aryRmvN(inputArray,deleteIndex,howDeleteIndex);
