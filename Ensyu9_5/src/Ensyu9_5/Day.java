...
public class Day {
	...
	if(inputMonth==1 || inputMonth==3 || inputMonth==5 || inputMonth==7 ||
		...
		//2月の場合
			...
			//うるう年の場合
			if(inputYear%baseNumberA==0 &&
					(inputYear%baseNumberB!=0 || inputYear%baseNumberC==0)) {
				do {
					//入力された値を読み込む
					whatDay = inputNumber.nextInt();
					//0以下と30以上が入力させた場合
	...
	//2つの日付を比較して
	//反転してれば１
	static final int compareBefore = 1;
	//同じなら２
	static final int compareSame   = 2;
	//正しい時系列なら０とする
	static final int compareAfter  = 0;
	/*
	 * 関数名:compareYear
	 * 概要:2つの日付の年を比較する
	 * 引数:開始日と終了日の年
	...
	public static int compareYear(int beforeYear, int afterYear){
		...
		if(beforeYear>afterYear) {
			//戻り値を１にする
			compareResult = compareBefore;
		}
		//同じ年の場合
		else if(beforeYear == afterYear) {
			//戻り値を２にする
			compareResult = compareSame;
		}
	...
	public static int compareMonth(int beforeYear, int afterYear, 
		...
		//同じ年のとき
		if(compareYear(beforeYear,afterYear)==compareBefore) {
			//未来のはずの月の方が過去の場合
	...
	public static boolean compareDay(int beforeYear,int beforeMonth,int beforeDay,
		...
		//開始日が終了日よりも未来の場合
		if(compareResult == compareBefore) {
			//判定結果をfalseに変える
			getAnswer = false;
		}
		//２つの日付が時間的に正しいか返す
